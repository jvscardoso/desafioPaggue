...
const Home = () => {
  ...
  const [thanksCompra, setThanksCompra] = useState(false)

  useEffect(() => {
    const handleConfirmarCompra = (data) => {
      console.log('Mensagem de confirmação de compra recebida:', data);
      setThanksCompra(true);
    };

    socket.on('confirmarCompra', handleConfirmarCompra);

    return () => {
      socket.off('confirmarCompra', handleConfirmarCompra);
    };
  }, []);
  ...
  return (
    <>
      {thanksCompra && <Message onClose={() => setThanksCompra(false)} />}
